# %load turbo_seti_sofia/find_event_pipeline.py
#!/usr/bin/env python
...
def find_event_pipeline(dat_file_list_str,
    ...
    for dat in dat_file_list:
        #print(dat)
        source_name = dat.split('_')[1]                #ss --> changed index from 5 to 1
    ...
    if on_source_complex_cadence != False:
        ...
        complex_cadence = False                                     #ss --> added else statement
    ...
    if user_validation == True:
        ...
        for i in range((int(n_files/number_in_cadence))):
            ...
            if complex_cadence == False:
                if on_off_first == 'ON':
                    name = file_sublist[0].split('_')[1]                      #ss --> chaned index
                    id_num = (file_sublist[0].split('_')[3]).split('.')[0]    #ss --> changed index 6 to 3
                if on_off_first == 'OFF':
